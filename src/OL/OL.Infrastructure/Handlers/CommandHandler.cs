...
namespace OL.Infrastructure.Handlers
{
        ...
        : ICommandHandler
    {
        ...
        {
            var aggregate = new OLProductAggregateRoot(command.Id, command.SystemId, command.Price, command.InstalmentMaxMouth, command.InstalmentMonthlyRepayment, command.SystemCategoryId, command.Translations.AsReadOnly());
            return categoryEventSourcingHandler.SaveAsync(aggregate);
        }
        ...
        public async Task HandleAsync(UpdateOlProductCommand command)
        {
            ...
            aggregate.UpdateCategory(command.Id, command.Price, command.InstalmentMaxMouth, command.InstalmentMonthlyRepayment, command.SystemCategoryId, command.Translations.AsReadOnly());
